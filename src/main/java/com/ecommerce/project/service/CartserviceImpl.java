...
public class CartserviceImpl implements CartService {
    ...
    public CartDTO updateProductQuantityInCart(Long productId, Integer quantity) {
        ...
        if(newQuantity == 0){
            deleteProductFormCart(cartId, productId);
        }else{
        ...
        CartDTO cartDTO = modelMapper.map(cart, CartDTO.class);
        List<CartItem> cartItems = cart.getCartItems();
        Stream<ProductDTO> productStream = cartItems.stream().map(item -> {ProductDTO prd =  modelMapper.map(item.getProduct(), ProductDTO.class);
    ...
    @Transactional
    @Override
    public String deleteProductFormCart(Long cartId, Long productId) {
        Cart cart = cartRepository.findById(cartId)
                .orElseThrow(() -> new ResourceNotFoundException("Cart", "cartId", cartId));
        CartItem cartItem = cartItemRepository.findCartItemByProductIdAndCartId(cartId, productId);
        if (cartItem == null) {
